...
Here you find the definitions for the UI.
It contains the logic of what happens if the user clicks on certain buttons, tic boxes etc.
*/
...
// I don't need the warnings, I know what I am doing.
	...
	_In_ int       nCmdShow)
{
	...
	if (!cmdParams.empty()) {
		...
		if (!cmdParams.compare(L"-dev")) {
			...
			std::wstring errorMsg(L"Invalid parameter: ");
			errorMsg.append(cmdParams);
...
INT_PTR CALLBACK mainProc(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static DFextractor* DFCurrInst = nullptr;
	...
	case WM_COMMAND:
		...
		case ID_FILE_OPENFILE:
		{
			...
			if (!(extrOptions & (~(1 << 3)))) {
				MessageBoxA(hDlg, "You must enable atleast one of the three extraction categories!", "Info", MB_OK | MB_ICONWARNING);
				break;
			}
			...
			if (!filePath.empty()) {
				...
				DFCurrInst = new DFextractor(filePath);
				if (DFCurrInst->lastError) {
					MessageBoxA(hDlg, DFCurrInst->GetDFerrorString(DFCurrInst->lastError), "Error", MB_OK | MB_ICONWARNING);
					break;
				}
				...
				if (DFCurrInst) {
					...
					if (!subfolder.empty()) {
						...
						DFCurrInst->changeCurrentOutputPath(subfolderFull);
